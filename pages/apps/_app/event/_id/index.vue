...
export default {
  ...
  methods: {
    ...
    goLive() {
      window.open(
        `apps/event/live/${this.$store.state.currentOrg.name}-${this.eventData.UniqLink}?e=${this.$route.params.id}&n=${this.eventData.Title}&o=${this.eventData.chatChannel}&p=${this.$config.rtmpURL}${this.eventData.id}`
      )
    },
    openPrintForm() {
      const logoUrl = `src="${
