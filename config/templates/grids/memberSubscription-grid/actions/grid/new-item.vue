    ...
    >
      <template v-slot:activator="{ on, attrs }">
        <v-btn text small v-bind="attrs" v-on="on" @click="dialog = true">
          <v-icon left>mdi-plus</v-icon> <i18n path="Common.NewSubscription" />
        </v-btn>
      </template>
      <v-card>
        <v-card-title
        ...
        >
          <h2 class="black--text pt-5 pb-4 font-weight-regular text-h5">
            <i18n path="Common.NewSubscription" />
          </h2>
          <v-spacer></v-spacer>
          <div>
        ...
        >
          <SaveButton
            ...
            color="primary"
            :action="onSave"
            :disabled="!valid || !datevalid || subscriptionPlan === ''"
            depressed
...
export default {
  ...
  methods: {
    ...
    async onSave() {
      ...
      this.formData.Cost = parseInt(this.formData.Cost) || null
      ...
      try {
        const res = await this.$axios.$post(`${url}ProductSubscriptions`, {
          ...this.formData,
        })
        if (res) {
          this.onClose()
        ...
        console.error(
          `Error in templates/grids/memberSubscription-grid/actions/grid/new-item.vue while making a POST call to ProductSubscriptions model from method onSave context:-URL:${url}`,
          err
        )
      }
    },
    ...
    async getProductsList() {
      ...
      try {
        ...
        console.error(
          `Error in templates/grids/memberSubscription-grid/actions/grid/new-item.vue while making a GET call to Product model from method getProductsList context:-URL:${url}`,
          err
        )
      }
    },
